...
RUN mkdir /release && \
    cp /build/libjpeg-turbo-java/build/libs/libjpeg-turbo*.jar /release && \
    cp -L /build/libjpeg-turbo-java/libjpeg-turbo/build/.libs/*.so /release

VOLUME [ "/export" ]
CMD tar -czf /export/libjpeg-turbo-java.tar.gz --directory=/release .
